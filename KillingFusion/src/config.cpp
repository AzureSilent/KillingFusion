...
// UnknownClipDistance = VoxelSize * 4
...
//目录设置
const std::string DATA_DIR = std::string("D:/Project/KillingFusion/data/");
const std::string OUTPUT_DIR = std::string("D:/Project/KillingFusion/data/output/");

const FusionTechnique fusionTechnique = KILLING_FUSION;
const DEFORMABLE_DATASET datasetType = SNOOPY;
const int numImageFiles[2] = {447, 630}; // 目下的图像总帧数
const std::string imageDir[2] = {std::string("Duck/"), std::string("Snoopy/")};
// 深度相机内参，必须是3*3矩阵
const std::string intrinsicParamsFile = "depthIntrinsics.txt"; 
...
const bool UpdateAllVoxelsInEachIter = true; //原作者设置的是false为了加快计算，但计算原理是不对的
 ...
 */
const int KILLING_MAX_ITERATIONS = 50;// 1024;
const double threshold = 0.000001;

const double alpha = 0.025;

// Killing weights
const double omegaKilling = 0.5; //0.04

// Killing - Purity
const double gammaKilling = 0.1; //0

// Level-set condition weights
const double omegaLevelSet = 0.2; //0.1
